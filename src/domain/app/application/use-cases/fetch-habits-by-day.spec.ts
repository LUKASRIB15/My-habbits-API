...
describe("FetchHabitsByDayUseCase", ()=>{
  ...
  beforeEach(()=>{
    ...
    sut = new FetchHabitsByDayUseCase(inMemoryHabitsRepository)
  ...
  it("should be able to get habits by day", async ()=>{
    ...
    expect(result.value?.habits).toHaveLength(2)
    expect(result.value?.habits).toEqual(expect.arrayContaining([
  ...
  it("should not be able to get habits by day of other client", async ()=>{
    ...
    expect(result.isSuccess()).toBeTruthy()
    expect(result.value?.habits).toHaveLength(0)
  })
})